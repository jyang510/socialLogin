...
class App extends Component {
  ...
  async getToken(authorizationCode, state) {
    ...
    console.log(`code의 값은 ${authorizationCode}`);
  ...
  componentDidMount() {
    ...
    if (authorizationCode && state) {
      this.getToken(authorizationCode, state);
    }
  }

  render() {
    const { isLogin, accessToken, refreshToken } = this.state;
    return (
      <Router>
        <div className='App'>
          {isLogin ? (
            <Mypage accessToken={accessToken} refreshToken={refreshToken} />
          ) : (
              <Login />
            )}
        </div>
